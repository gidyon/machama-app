...
var (
	TransactionAPIServer *transactionAPIServer
...
var _ = BeforeSuite(func() {
	...
	// Create ussdlog API
	TransactionAPI, err = NewTransactionAPI(ctx, opt)
	Expect(err).ShouldNot(HaveOccurred())

	var ok bool
	TransactionAPIServer, ok = TransactionAPI.(*transactionAPIServer)
	Expect(ok).Should(BeTrue())

	// Load data
	Expect(laodMockData(100)).ShouldNot(HaveOccurred())

	_, err = NewTransactionAPI(ctx, nil)
	Expect(err).Should(HaveOccurred())

	opt.SQLDB = nil
	_, err = NewTransactionAPI(ctx, opt)
	Expect(err).Should(HaveOccurred())
