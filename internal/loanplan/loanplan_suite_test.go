...
func TestChama(t *testing.T) {
	...
	RunSpecs(t, "Loan Product Suite")
}

var (
	LoanProductAPIServer *loanProductAPIServer
	LoanProductAPI       loan.LoanProductAPIServer
...
var _ = BeforeSuite(func() {
	...
	// Create ussdlog API
	LoanProductAPI, err = NewLoanProductAPI(ctx, opt)
	Expect(err).ShouldNot(HaveOccurred())

	var ok bool
	LoanProductAPIServer, ok = LoanProductAPI.(*loanProductAPIServer)
	Expect(ok).Should(BeTrue())

	// Load data
	Expect(laodMockData(100)).ShouldNot(HaveOccurred())

	_, err = NewLoanProductAPI(ctx, nil)
	Expect(err).Should(HaveOccurred())

	opt.SQLDB = nil
	_, err = NewLoanProductAPI(ctx, opt)
	Expect(err).Should(HaveOccurred())
